                ...
                if(exist_MAC_address_row->uuid_record_table_array[i].valid==1){
                    ...
                    if(time_gap > 0){
                        ...
                        for(j=0;j<time_gap;j++){
                            ...
                            head++;
                            if(head == NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING){
                                head=0;
                            }
...
int rssi_weight(int * rssi_array,int k,int head, int rssi_weight_multiplier, int unreasonable_rssi_change){
    
    if(rssi_array[k]==0) 
        return 0;

    //Ignore unreasonable rssi.
    if((k + NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING - 1) % 
        NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING != head ){     
    
        if(rssi_array[(k + NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING - 1 ) 
           % NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING ] != 0 ){

            if(abs(rssi_array[k] - 
               rssi_array[( k + NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING - 1 ) 
               % NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING ]) > unreasonable_rssi_change){
                return 0;
            }
        }
    }
    ...
    for (i = 0; i < size; i++) {
        ...
        while (curr != 0) {//every mac      
            ...
            for(l=0;l<table_row->record_table_size;l++){
                if(table_row->uuid_record_table_array[l].valid!=0 &&
                   strcmp(table_row->uuid_record_table_array[l].uuid,table_row->summary_uuid)==0){
                        ...
                        for(k=0;k < NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING;k++){
                            if(rssi_weight(table_row->uuid_record_table_array[l].rssi_array,
                                           k,table_row->uuid_record_table_array[l].head, 
                                           rssi_weight_multiplier,
                                           unreasonable_rssi_change)>0){
            ...
            while(j<table_row->record_table_size){//不同uuid
                ...
                for(k=0;k<NUMBER_OF_RSSI_SIGNAL_UNDER_TRACKING;k++){
                    //check is reasonable
                    weight=rssi_weight(table_row->uuid_record_table_array[j].rssi_array,
                                       k,
    ...
    if (res == 0 ) {
        ...
        if (new_head != 0 ) {           
            ...
            */
            ...
            hash_table_row_for_new_MAC->record_table_size=NUMBER_OF_LBEACON_UNDER_TRACKING;
            ...
            hash_table_row_for_new_MAC->record_table_size=NUMBER_OF_LBEACON_UNDER_TRACKING;
            for(i=1;i<NUMBER_OF_LBEACON_UNDER_TRACKING;i++){
